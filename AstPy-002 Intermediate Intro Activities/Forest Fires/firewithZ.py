...
class CA:
    ...
    def __init__(self, grid, edge_rule, verbose=False, plot=False, plot3D=False, 
                gif=False, gif3D=False, dpi=100, **kwargs):
        """ Initialises the CA
        """
        ...
        if self.edge_rule == 'ghost':
            for index, val in np.ndenumerate(self.grid):
                ...
                if 0 not in index and self.size+1 not in index:
                    new_grid[i, j] = rule(i, j, val, self.get_neighbours(i, j, 1), **kwargs)
        self.grid = new_grid
        self.t += 1
...
# custom fire rule
def fire_rule_Z(i, j, val, n, **kwargs):
    """ Fire rule
    ...
    """
    p_burn, p_lightning, terrain, d, p_water = kwargs.values()
    if val == -1:
    ...
    if random() < p_burn*opts[1]/w_supress:
        return 1 # burning
...
def forest_3D(t, state, terrain, colours, file_path, dpi):
    ...
    ax.plot_surface(X, Y, terrain, facecolors=color_map, edgecolor='none', alpha=.7)
    ax.axis('off')
...
if __name__ == "__main__":
    ...
    forest_colours = ["dodgerblue", "forestgreen", "orange", "dimgrey"]
    forest_cmap = ListedColormap(forest_colours)
    bounds = [-1.1, -0.5, .5, 1.5, 2.5]
    ...
    # Z is our terrain map
    ln = len(terrain_pad)
    terrain = terrain_pad[1:ln+1, 1:ln+1]

    # where Z is 0 set initial to -1 for water
    ...
    fire.mevolve(fire_rule_Z, steps, p1=p_burn, p2=p_lightning, Z=terrain_pad, d=d_water, p3=p_water)
    fire.make_gif()
    fire.make_gif3D()