...
for i in range(len(names)):
    if (moon[i] == 'n') and (names[i] != 'Pluto'):
        ax.scatter(avg_dist[i], radii[i], color=colors[i], label=names[i], sizes=[sizes[i]], alpha=0.75, ec='None', zorder=5)
...
for i in range(len(disc_meths)):
    ...
    ex_rad = newdf['pl_radj'].values/(12756/2)*(142984/2)
    ex_orb = newdf['pl_orbsmax'].values
    if disc_meths[i] != 'Transit':
        alpha=0.5
        zorder=1
    else:
        alpha=0.05
...
ax.set_title('Radius relative to the Earths vs semi-major axis, Kepler exoplanet data is also shown', color='grey', pad=20)
...
# plt.show()
plt.savefig('radius_vs_distance_exoplanets.png', dpi=150)