...
saturn_rings = df.loc[df['object'] == 'neptune']

print(saturn_rings)

names = saturn_rings['ring_name'].values
...
rad = 49528/2 # km
circle = Circle((0, 0), rad, color='royalblue', zorder=z, alpha=1, ec='None')
...
for i in range(len(names)):
    ...
    z -= 1
    # second ring circle
    if names[i] not in ['Arago', 'Adams (N63)']:
        circle = Circle((0, 0), outer_ring, color='cornflowerblue', zorder=z, alpha=alphas[i], ec='None')
        ax.add_artist(circle)
    else:
        circle = Circle((0, 0), outer_ring, color='k', zorder=z*2, alpha=alphas[i], fc='None')
        ax.add_artist(circle)
    z -=1
...
moonfile = '../data/neptune_moons.csv'
moons = pd.read_csv(moonfile)

moons = moons.loc[moons['Semimajor Axis'] <= moons.loc[moons['Name'] == 'Larissa', 'Semimajor Axis'].iloc[0]]
...
ax.axhline(72000, 0.1, 0.3, color='darkgray', lw=1, zorder=31)
ax.text(0.2, 0.949, '{:,.1f} km'.format(scale), color='darkgray', fontsize=10, zorder=31, transform=ax.transAxes, ha='center')

ax.text(0.87, 0.03, 'Moon sizes are shown relative to each\nother but not to Neptune and the rings.\nOpacity is relative to the optical depth.\nThe Arago \& Adams rings opacities have\nbeen increased for visibility. The Adams\nring is 10 times thinner than shown.', color='darkgray', fontsize=8, zorder=31, transform=ax.transAxes, ha='center', multialignment='right', alpha=.7)

ax.set_aspect('equal')
ax.axis('off')

plt.show()
# plt.savefig('../figures/neptune_rings.png', dpi=150, bbox_inches='tight')