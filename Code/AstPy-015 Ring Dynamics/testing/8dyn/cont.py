...
def plotParticles(sim, k):
    ...
    for i, p in enumerate(sim.particles):
        ...
        ax.add_patch(circ)
    plt.savefig('./8dyn/dynamics_'+str(k)+'.png', dpi=300)
    # plt.show()
    fig.clf()
    plt.close()
...
def plotHistA(sim, k):
    ...
    ax = plt.subplot(111)
    freq, bins = np.histogram(A, bins=range(l, h+10, 10))
    top_freq = 100
    ...
    left=False,       
    labelleft=False) 
    plt.savefig("./8dyn/a_dist_"+str(k)+".png", dpi=300)
    fig.clf()
    plt.close()
...
sim = rebound.Simulation("8dyn.bin")
...
for i in range(200):
    ...
    plotParticles(sim, i+201)
    plotHistA(sim, i+201)

get_time()

sim.save("8dynCont2.bin")