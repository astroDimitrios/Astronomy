...
# mass_tot = 0
# while count < 10000:
#     radius = powerlaw(slope=-4, min_v=1, max_v=4)/10000  # [m]
#     mass = particle_density*4./3.*np.pi*(radius**3)
#     r = np.random.uniform()
#     rs = r*(h-l) + l
...
def plotParticles(sim, k):
    ...
    for i, p in enumerate(sim.particles):
        ...
        ax.add_patch(circ)
    plt.savefig('./8dyn3MimasOnly/dynamics_'+str(k)+'.png', dpi=300)
    # plt.show()
    fig.clf()
    plt.close()
...
def plotHistA(sim, k):
    ...
    ax = plt.subplot(111)
    freq, bins = np.histogram(A[2:], bins=range(70000, 150000+10, 10))
    top_freq = 100
    ...
    left=False,       
    labelleft=False) 
    plt.savefig("./8dyn3MimasOnly/a_dist_"+str(k)+".png", dpi=300)
    fig.clf()
    plt.close()
...
sim = rebound.Simulation("8dyn3MimasOnlyCont16.bin")
...
for i in range(2000):
    ...
    plotParticles(sim, i+1801)
    plotHistA(sim, i+1801)

get_time()

sim.save("8dyn3MimasOnlyCont26.bin")