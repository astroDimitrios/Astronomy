...
while count < 100:
    ...
    # ts = np.sqrt((rs*1000)**3)
    # v = np.pi*rs*2/ts*10
    v = np.sqrt(6.67428e-11 * sim.particles[0].m/ (rs*1000)) / 1000 *.75# km/s
    print(v)
...
def plotParticles(sim, k):
    ...
    for i, p in enumerate(sim.particles):
        ...
        ax.add_patch(circ)
    plt.savefig('./figures10/dynamics_'+str(k)+'.png', dpi=200)
    # plt.show()
    fig.clf()
    plt.close()
...
def plotHistA(sim, k):
    ...
    ax.set_xlim(l, h)
    plt.savefig("./figures10/a_dist_"+str(k)+".png")
    fig.clf()
    plt.close()
