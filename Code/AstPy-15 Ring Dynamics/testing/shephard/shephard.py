...
while count < 10000:
    ...
    mass = particle_density*20*4./3.*np.pi*(radius**3)
    r = np.random.uniform()
    # while .4 < r < .6:
    #     r = np.random.uniform()
    if .4 < r < .5:
        ...
        r = .6 
    theta = np.random.uniform(low=-1, high=1)/40+np.pi/2
    if count == 0:
        radius = radius*100
        mass = particle_density*4./3.*np.pi*(radius**3)
        r = 0.5
    ...
    # ts = np.sqrt(r**3)
    # v = np.pi*r*2/ts
    v = np.sqrt(6.67428e-11 * sim.particles[0].m/ (rs*1000)) / 1000# km/s
...
def plotParticles(sim, k):
    ...
    # trans = transforms.Affine2D().rotate_deg(k*3.6) + ax.transData
    for i, p in enumerate(sim.particles):
        if i == 0:
            fc, ec, a, r = "goldenrod", "None", 1, rads[0]
        elif i == 1:
            fc, ec, a, r = "k", "None", 1, p.r/4
            # print(p.x)
            # print(p.y)
            ax.set_xlim(p.x-2000, p.x+2000)
            ax.set_ylim(p.y-2000, p.y+2000)
        else:
            fc, ec, a, r = "lightgray", "k", 1, p.r
        circ = Circle((p.x, p.y), r, facecolor=fc, edgecolor=ec, alpha=a)
        # circ.set_transform(trans)
        ax.add_patch(circ)
    plt.savefig('./shephard/dynamics_'+str(k)+'.png', dpi=300)
    # plt.show()
    fig.clf()
    plt.close()
...
def plotHistA(sim, k):
    ...
    ax = plt.subplot(111)
    freq, bins = np.histogram(A[2:], bins=range(l, h+10, 10))
    top_freq = 100
    ...
    left=False,       
    labelleft=False) 
    plt.savefig("./shephard/a_dist_"+str(k)+".png", dpi=300)
    fig.clf()
    plt.close()
