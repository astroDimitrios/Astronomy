...
def plotParticles(sim, k):
    ...
    for i, p in enumerate(sim.particles):
        if i == 0:
            fc, ec, a, r = "goldenrod", "None", 1, p.r
        elif i == 1:
            fc, ec, a, r = "dimgrey", "None", 1, p.r*10/2
        else:
            fc, ec, a, r = "lightgray", "k", 1, p.r*100
        circ = Circle((p.y, p.x), r, facecolor=fc, edgecolor=ec, alpha=a)
        ax.add_patch(circ)
    plt.savefig('./figures9/dynamics_'+str(k)+'.png')
    # plt.show()
    fig.clf()
    plt.close()
...
def plotHistA(sim, k):
    ...
    left=False,       
    labelleft=False) 
    plt.savefig("./figures9/a_dist_"+str(k)+".png")
    fig.clf()
    plt.close()
...
sim.save("mimSat5orbitsMimas100thStepWithRadii.bin")
# del sim
