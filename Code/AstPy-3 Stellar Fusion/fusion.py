...
ax.annotate("Fe", xy=(df["Z"][indFe], df["BINDING ENERGY (keV)"][indFe]-.24), xytext=(df["Z"][indFe], df["BINDING ENERGY (keV)"][indFe]-1.74), arrowprops=dict(arrowstyle="->", fc="grey", ec="grey"))
ax.annotate("H", xy=(df["Z"][0], df["BINDING ENERGY (keV)"][0]), xytext=(df["Z"][0]+1.25, df["BINDING ENERGY (keV)"][0]+.25))
ax.annotate("He", xy=(df["Z"][1], df["BINDING ENERGY (keV)"][1]), xytext=(df["Z"][1], df["BINDING ENERGY (keV)"][1]+.35))
ax.annotate("Li", xy=(df["Z"][2], df["BINDING ENERGY (keV)"][2]), xytext=(df["Z"][2]+1.25, df["BINDING ENERGY (keV)"][2]-.35))
ax.annotate("Be", xy=(df["Z"][3], df["BINDING ENERGY (keV)"][3]), xytext=(df["Z"][3]+1.25, df["BINDING ENERGY (keV)"][3]-.4))
ax.annotate("B", xy=(df["Z"][4], df["BINDING ENERGY (keV)"][4]), xytext=(df["Z"][4]+1.25, df["BINDING ENERGY (keV)"][4]-.2))
...
def animate(i):
    ...
    if not df["EL"][i] in alreadyAnnotated:
        element.remove()
        element = ax.annotate(df["EL"][i], xy=(df["Z"][i], df["BINDING ENERGY (keV)"][i]), xytext=(df["Z"][i], df["BINDING ENERGY (keV)"][i]+.5))
    elif df["EL"][i]=="N":
        element.remove()
        element = ax.annotate(df["EL"][i], xy=(df["Z"][i], df["BINDING ENERGY (keV)"][i]), xytext=(df["Z"][i]-0.75, df["BINDING ENERGY (keV)"][i]+.5))
    else:
        element.remove()
        element = ax.annotate("", xy=(0,0))
    BEText.remove()
...
# anim.save('bindingEnergy.gif', writer='imagemagick', fps=2)
plt.show()
plt.close(1)
